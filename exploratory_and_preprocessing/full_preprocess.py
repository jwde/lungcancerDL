...
def resample(image, scan, new_spacing=[1,1,1], final_shape=[25,50,60]):
    ...
    image = scipy.ndimage.interpolation.zoom(image, real_resize_factor, mode='mirror', order = 1)
    
    final_resize_factor = final_shape / new_shape
    image = scipy.ndimage.interpolation.zoom(image, final_resize_factor, mode='mirror', order = 1)
...
def main_loop(INPUT_FOLDER,OUT_FOLDER,patient,npat, i):
    ...
    #print (trimmed_pixels.shape)
    Z, X, Y = trimmed_pixels.shape
    #pix_resampled = resample(trimmed_pixels, patientINFO,[2,2,2], [Z,227,227])
    pix_resampled = resample(trimmed_pixels, patientINFO,[2,2,2], [60,227,227])
    pix_resampled[pix_resampled < min_thresh] = err_val
    pix_resampled[pix_resampled > max_thresh] = max_thresh
    outfile = os.path.join(OUT_FOLDER, '%s.npy' % patient)
    np.save(outfile, pix_resampled)

num_cores = 8
# Run parallelized loop over all patients
Parallel(n_jobs=num_cores)(delayed(main_loop)(INPUT_FOLDER,OUT_FOLDER,patients[i],npat, i) for i in iterations)
