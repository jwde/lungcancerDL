...
class Shift(object):
    ...
    def __call__(self, img):
        if not isinstance(img, np.ndarray):
            print ("Calling To Tensor on Non numpy array")
        return ndimage.shift(img, self.shift, order = 0)
...
class RandomShift(object):
    ...
    def __call__(self, img):
        ...
        return ndimage.shift(img, shift, order = 0)
...
class RandomRotation(object):
    ...
    def __call__(self, img):
        ...
        return ndimage.interpolation.rotate(img, rot, (2, 1), reshape = False, order = 0)
