...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    dscores = probs
    dscores[range(N), y] -= 1
    grads['W2'] = np.dot(np.transpose(f1), dscores) / N + reg * W2
    grads['b2'] = np.sum(dscores, axis=0) / N
    d1 = np.dot(dscores, np.transpose(W2))
    d1[f1 <= 0] = 0
    grads['W1'] = np.dot(np.transpose(X), d1) / N + reg * W1
  ...
  def predict(self, X):
    ...
    y_pred = np.argmax(np.dot(l1, W2) + b2, axis=1)
